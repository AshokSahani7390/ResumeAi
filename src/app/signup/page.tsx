  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 500) {
          localStorage.setItem("resumeai_user", JSON.stringify({ email, name }));
    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--color-bg)] px-4">
      ...
      <div className="w-full max-w-md slide-up">
        <div className="text-center mb-8">
          ...
          <p className="text-[var(--color-text-muted)] text-sm">Start building your perfect resume in minutes</p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-8 shadow-lg">
          ...
          <form onSubmit={handleSignup} className="space-y-5">
            <div>
              ...
              <input type="text" required value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-[var(--color-border)] bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm" placeholder="John Doe" />
            </div>
            <div>
